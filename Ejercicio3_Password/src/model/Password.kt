...
class Password()
{
    ...
    fun generarPassword()
    {
        ...
        val num = "0123456789"
        var ind = Random.nextInt(0, 2)
        for (i in 0..longitud-1)
        {
            when (ind)
            {
                0 -> contrasenia+= LETRAS[Random.nextInt(0, 26)]
                1 -> contrasenia+= letras[Random.nextInt(0, 26)]
                2-> {contrasenia+= num[Random.nextInt(0, 10)];}
            }
            ind = Random.nextInt(0, 3)
        }
    }
